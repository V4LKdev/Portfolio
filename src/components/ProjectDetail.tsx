  ...
  const renderDesignContent = () => (
    <div className="space-y-8 w-full max-w-none">
      ...
      {project.design?.challenges && project.design.challenges.length > 0 && (
        <div>
          <h4 className="text-xl font-semibold text-amber-100 mb-3">
            Design Challenges
          </h4>
      ...
      <div className="bg-black/30 p-6 rounded-lg border border-amber-500/20">
        ...
        <p className="text-amber-200/80 mb-4">
          This section contains detailed design documentation, wireframes, and
          visual concepts that guided the development process.
        </p>
      ...
      <div>
        ...
        <p className="text-amber-200/80 text-lg leading-relaxed mb-6">
          {project.code?.architecture ||
            "Technical architecture will be documented here..."}
        </p>
      </div>
        ...
        <div className="w-full">
          ...
          <div className="space-y-4 w-full">
            {project.code.codeSnippets.map((snippet, index) => (
              <div
                key={index}
                className="bg-black/40 rounded-lg border border-amber-500/20 w-full"
              >
                <div className="bg-amber-500/10 px-4 py-2 border-b border-amber-500/20">
                  <h5 className="text-amber-100 font-medium">
                    {snippet.title}
                  </h5>
  ...
  const renderImplementationContent = () => (
    <div className="space-y-8 w-full max-w-none">
      <div>
        ...
        <p className="text-amber-200/80 text-lg leading-relaxed mb-6">
          {project.implementation?.process ||
            "Development process documentation will be added here..."}
        </p>
      </div>

      {project.implementation?.timeline &&
        project.implementation.timeline.length > 0 && (
          <div>
            <h4 className="text-xl font-semibold text-amber-100 mb-3">
            ...
            <div className="space-y-3">
              {project.implementation.timeline.map((phase, index) => (
                <div
                  key={index}
                  className="bg-black/30 p-4 rounded-lg border border-amber-500/20"
                >
      ...
      {project.implementation?.challenges &&
        project.implementation.challenges.length > 0 && (
          <div>
            <h4 className="text-xl font-semibold text-amber-100 mb-3">
      ...
      {project.implementation?.results &&
        project.implementation.results.length > 0 && (
          <div>
            <h4 className="text-xl font-semibold text-amber-100 mb-3">
      ...
      <div className="bg-black/30 p-6 rounded-lg border border-amber-500/20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-amber-500/5 p-4 rounded border border-amber-500/10">
            <h5 className="text-amber-100 font-medium mb-2">
              Future Improvements
            </h5>
            <p className="text-amber-200/70 text-sm">
        ...
        <div className="p-8">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {project.tags.map((tag) => (
              <span
                key={tag}
                className="px-3 py-1 bg-amber-500/20 text-amber-200 text-sm rounded-full"
              >
                {tag}
              </span>
            ))}
