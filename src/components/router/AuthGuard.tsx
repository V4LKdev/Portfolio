...
const AuthGuard: React.FC<AuthGuardProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const hasPassedWelcomeThisSession = sessionStorage.getItem(SESSION_WELCOME_KEY) === 'true';
    
    // Logic to direct users based on session status
    if (!hasPassedWelcomeThisSession && location.pathname !== '/welcome' && location.pathname !== '/goodbye') {
