...
export const THEMES: Record<string, ThemeConfig> = {
  ...
  warm: {
    id: 'warm',
    name: 'Deadlock Warm',
    description: 'Warm amber theme inspired by game UIs like Deadlock/Overwatch',
    colors: {
      background: '8 8% 8%',
      foreground: '45 15% 85%',
  ...
  cool: {
    id: 'cool',
    name: 'Cyber Cool',
    description: 'Cool blue theme with modern white text and cyberpunk aesthetics',
    colors: {
      background: '220 15% 6%',
...
export function themeToCSSProperties(theme: ThemeConfig): Record<string, string> {
  ...
  Object.entries(theme.colors).forEach(([key, value]) => {
    ...
    if (key.startsWith('sidebar')) {
      ...
      cssProps[`--sidebar-${sidebarKey === 'background' ? 'background' : sidebarKey}`] = value;
    } else {
...
export function applyTheme(themeId: string): void {
  ...
  root.style.setProperty('--theme-social-icon-hover', theme.gameStyle.menuHoverColor);
